...
#   ./run_tests.sh --balance       # Run only the 250-stage balance-curve regression probe
...
if [ "$RUN_UNIT" = true ]; then
    echo -e "\n${YELLOW}[1/5] Running Unit & Integration Tests (tests/test_runner.gd)...${NC}"
...
if [ "$RUN_SMOKE" = true ]; then
    echo -e "\n${YELLOW}[2/5] Running UI Smoke & Interaction Tests (tests/ui_smoke.gd)...${NC}"
...
if [ "$RUN_E2E" = true ]; then
    echo -e "\n${YELLOW}[3/5] Running E2E Campaign Playthrough Bot (tests/e2e_playthrough.gd)...${NC}"
...
# 4. Balance Curve Regression Probe
if [ "$RUN_BALANCE" = true ]; then
    echo -e "\n${YELLOW}[4/5] Running Balance Curve Regression Probe (tests/balance_probe.gd)...${NC}"
    godot --headless --path "${GODOT_DIR}" -s tests/balance_probe.gd
    echo -e "${GREEN}✓ Balance curve regression floor held!${NC}"
fi

# 5. GUT (Godot Unit Test) Suite
if [ "$RUN_GUT" = true ]; then
    echo -e "\n${YELLOW}[5/5] Running GUT Test Suite (tests/gut/test_*.gd)...${NC}"
...
# 9. Mobile Visual Snapshots (Optional or Flagged)

...
func create(seed: int, encounter: Dictionary, deck: Array, player_health: int, upgrades := {}, equipment := [], card_runes := {}, modifier := {}, relics := [], hero_bonuses := {}, equipment_tiers := {}, equipment_inscriptions := {}) -> Dictionary:
	...
	state = {
		...
		"last_element":"",
		"stats":{"damage_dealt":0,"cards_played":0,"shield_gained":0,"card_play_counts":{}}
	}
...
func play(hand_index: int, target_index := -1) -> bool:
	...
	if state.has("stats"):
		...
		if not state.stats.get("card_play_counts") is Dictionary: state.stats.card_play_counts = {}
		state.stats.card_play_counts[card.id] = int(state.stats.card_play_counts.get(card.id, 0)) + 1

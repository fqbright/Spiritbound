...
| `Godot/tools/ui_audit.gd` | **Measures** the nine snapshot screens instead of eyeballing them: hit targets under 44pt, text cut off by its own container, and text contrast against the rendered pixels behind it. Asserts nothing, gates nothing — it prints numbers. Run it with a real renderer (`godot --path Godot/ --script res://tools/ui_audit.gd`) when changing layout, colours, or type sizes. See `Docs/UI_AUDIT.md` for the current numbers and the two metric mistakes worth avoiding if you extend it. |
| `Docs/ARCHITECTURE.md` | Why the code is shaped this way. Read it before a structural change. |
| `Docs/UI_AUDIT.md` | Measured readability/size state of every screen, what was fixed and why, and what is still open. Read before claiming a screen "looks fine". |
...
| `Docs/LAUNCH_READINESS.md` | **Hard gate on submitting/resubmitting this app.** Its three code sections now exist in-repo — account deletion (§1), a real purchase gate for the season pass (§2), and crash/error logging + minimal analytics (§3) — but every one of them still has a non-code Definition of Done that needs Supabase / App Store Connect credentials (deploy the two Edge Functions, create the `client_events` / `entitlements` tables, vendor a billing plugin and wire it via `PurchaseService.set_provider()`, configure the store products, and set the Apple/Google verification secrets). Read it before any App Store/Play Store submission work, and before touching `game.gd`'s `season_pass`/`is_premium` handling or anything account-deletion-adjacent. `Docs/STORE_SUBMISSION.md` is the store-side companion checklist. |
